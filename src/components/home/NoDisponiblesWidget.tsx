...
export interface NoDisponiblesWidgetDish {
  id: number
  name: string
  category: FamilyType
}

interface NoDisponiblesWidgetProps {
  dishes: NoDisponiblesWidgetDish[]
}

export const NoDisponiblesWidget = ({ dishes }: NoDisponiblesWidgetProps) => {
  return (
    <Link
      href={HOME_WIDGET_LINKS.noDisponibles}
      className="flex-1 min-w-[18rem] max-w-md bg-not-available-main rounded-xl p-4 shadow-lg flex flex-col gap-3 text-white hover:brightness-105 transition-transform"
    >
      <h3 className="uppercase font-bold text-xl text-center">NO DISPONIBLES</h3>

      {dishes.length === 0 ? (
        <p className="text-center uppercase text-sm opacity-90">
          No hay platos no disponibles
        </p>
      ) : (
